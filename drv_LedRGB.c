...
//  numPWM_red  numPWM_green  numPWM_blue    LounchBy          mode          state     period   startTime    red   green   blue  Rslide  Gslide  Bslide 
	    { 0,           0,           0,       SWITCH_BY_HIGH,  mode_BLINK,    mode_OFF,    1000,       0,       255,   255,   255,    0,      0,      0}// rgb_INDICATE
 };
 ...
 void drv_LedRGB_Init(TIM_HandleTypeDef* timer_Red, uint8_t channel_Red,
	 ...
	 if(timer_Red != NULL && timer_Green!= NULL && timer_Blue != NULL &&
	    ...
	    lastLedNumber < rgb_Amount){
			...
			rgbLeds[lastLedNumber].turnOnMode = turnOnMode;
...
//
void  drv_LedRGB_SetDefinedColor(rgbLeds_t led, uint8_t  color){
